...
local SINGLE_LIGHT_GLARE_OVERLAY_PROFILE = {
    overlayR = 0.00,
    overlayG = 0.10,
    overlayB = 0.00,
    overlayAlpha = 0.1,
...
NV.RegisterNightVisionMode({
    id = (NV.Modes and NV.Modes.MODE_SINGLE_LIGHT_Glare) or "single_light_glare",
    label = "Single Point Light /w Glare",
    source = "NVFramework",
    priority = 2,
    usesPlayerPointLight = true,
    overlayProfile = SINGLE_LIGHT_GLARE_OVERLAY_PROFILE,
    ...
    activate = function(player, options)
        ...
        if NV.Overlay and NV.Overlay.ApplyProfile then
            NV.Overlay.ApplyProfile(SINGLE_LIGHT_GLARE_OVERLAY_PROFILE)
        end
