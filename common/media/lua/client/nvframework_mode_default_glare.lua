...
local CUSTOM_OVERLAY_PROFILE_Glare = {
    overlayR = 0.00,
    overlayG = 1.00,
    overlayB = 0.00,
    overlayAlpha = 0.25,
...
local CUSTOM_LIGHT_PROFILE_Glare = {
    maxDistance = 12,
    step = 0.4,
    radius = 3.5,
    legacyRadius = 1,
...
NV.RegisterNightVisionMode({
    id = (NV.Modes and NV.Modes.MODE_CUSTOM_Glare) or "custom_glare",
    label = "Default /w Glare",
    source = "NVFramework",
    priority = 0,
    usesDirectionalLight = true,
    overlayProfile = CUSTOM_OVERLAY_PROFILE_Glare,
    lightProfile = CUSTOM_LIGHT_PROFILE_Glare,
    ...
    activate = function(player, options)
        ...
        if NV.Overlay and NV.Overlay.ApplyProfile then
            NV.Overlay.ApplyProfile(CUSTOM_OVERLAY_PROFILE_Glare)
        end
        ...
        if NV.LightController and NV.LightController.ApplyProfile then
            NV.LightController.ApplyProfile(CUSTOM_LIGHT_PROFILE_Glare)
        end
