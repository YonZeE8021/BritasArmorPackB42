...
local PHOSPHOR_OVERLAY_PROFILE = {
    ...
    noiseB = 0.76,
    noiseTexturePaths = {
        "media/textures/NVFramework/Overlay/nvo1_w.png",
    },
    vignetteSize = 350,
    vignetteAlpha = 0.55,
    vignetteLayers = 4,
...
NV.RegisterNightVisionMode({
    id = "nvframework_phosphor_overlay",
    label = "White Phosphor Overlay",
    source = "NVFramework",
    priority = 5,
    usesPlayerPointLight = true,
    overlayProfile = PHOSPHOR_OVERLAY_PROFILE,
    ...
    activate = function(player, options)
        ...
        if NV.Overlay and NV.Overlay.ApplyProfile then
            NV.Overlay.ApplyProfile(PHOSPHOR_OVERLAY_PROFILE)
        end
